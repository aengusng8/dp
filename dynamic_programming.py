...
def solve_tsp_dynamic(cities):
    ...
    for m in range(2, len(cities)):
        cities_b = {}
        for cities_set in [
            frozenset(C) | {0} for C in itertools.combinations(range(1, len(cities)), m)
        ]:
            for j in cities_set - {0}:
                cities_b[(cities_set, j)] = min(
                    [
                        (
                            cities_a[(cities_set - {j}, k)][0] + distance_matrix[k][j],
    ...
    res = min(
        [
            (cities_a[d][0] + distance_matrix[0][d[1]], cities_a[d][1])
...
if __name__ == "__main__":
    ...
    print("Shortest path :", *g, sep=' -> ')
    print("Minimum path cost :", path_cost(sol))
    visualize_tsp("Travelling Salesman Problem: Dynamic Programming", sol)
